...
ax.set_xlim(
    left=starting_x, right=max(function_points) + 2
...
htext_best = ax.text(
    0, 0, "Best Case Effort", color="green", fontsize=8, ha="left", visible=False
)
(hline_best_adj,) = ax.plot([], [], color="green", linestyle="-", visible=False)
htext_best_adj = ax.text(
    0, 0, "Best Case Effort (adj)", color="green", fontsize=8, ha="left", visible=False
)
(hline_worst,) = ax.plot([], [], color="red", linestyle="-", visible=False)
htext_worst = ax.text(
    0, 0, "Worst Case Effort", color="red", fontsize=8, ha="left", visible=False
)
(hline_worst_adj,) = ax.plot([], [], color="red", linestyle="-", visible=False)
htext_worst_adj = ax.text(
    0, 0, "Worst Case Effort (adj)", color="red", fontsize=8, ha="left", visible=False
)
(hline_likely,) = ax.plot([], [], color="blue", linestyle="-", visible=False)
htext_likely = ax.text(
    0, 0, "Likely Effort", color="blue", fontsize=8, ha="left", visible=False
)
...
vtext_c = ax.text(
    ...
    "",
    color="black",
    bbox=dict(facecolor="white", edgecolor="black", boxstyle="square", linewidth=1),
    fontsize=8,
    ha="center",
    visible=False,
)
(vline_c_adj_b,) = ax.plot([], [], color="gray", linestyle="-", visible=False)
vtext_c_adj_b = ax.text(
    0,
    0,
    "",
    color="black",
...
(bline,) = ax2.plot(
    [], [], color="blue", linestyle="-", marker=".", label="Effort Value", visible=False
)
(tline,) = ax2.plot(
    [],
    [],
...
def find_worst_effort(
    function_points, effort_days, specified_function_point, likely_effort, direction="L"
):
...
def on_click(event):
    ...
    if event.inaxes == ax:
        ...
        wc_ef_left, wc_fp_left = find_worst_effort(
            function_points, effort_days, clicked_x, likely_effort
        )
        wc_ef_right, wc_fp_right = find_worst_effort(
            function_points, effort_days, clicked_x, likely_effort, direction="R"
        )
        if wc_ef_left == 0:
            ...
            worst_case_effort = find_y_value(
                wc_fp_left, wc_ef_left, wc_fp_right, wc_ef_right, clicked_x
            )
        ...
        wc_ef_left, wc_fp_left = find_worst_effort(
            function_points, effort_days, wc_adj_fp, likely_effort
        )
        wc_ef_right, wc_fp_right = find_worst_effort(
            function_points, effort_days, wc_adj_fp, likely_effort, direction="R"
        )
        if wc_ef_left == 0:
            ...
            worst_case_effort_adj = find_y_value(
                wc_fp_left, wc_ef_left, wc_fp_right, wc_ef_right, wc_adj_fp
            )

        # Determine the best-case scenario based on the clicked x-coordinate
        bc_ef_left, bc_fp_left = find_best_effort(
            function_points, effort_days, clicked_x, likely_effort
        )
        bc_ef_right, bc_fp_right = find_best_effort(
            function_points, effort_days, clicked_x, likely_effort, direction="R"
        )
        if bc_ef_left == 0:
            ...
            best_case_effort = find_y_value(
                bc_fp_left, bc_ef_left, bc_fp_right, bc_ef_right, clicked_x
            )
        ...
        bc_ef_left, bc_fp_left = find_best_effort(
            function_points, effort_days, bc_adj_fp, likely_effort
        )
        bc_ef_right, bc_fp_right = find_best_effort(
            function_points, effort_days, bc_adj_fp, likely_effort, direction="R"
        )
        if bc_ef_left == 0:
            ...
            best_case_effort_adj = find_y_value(
                bc_fp_left, bc_ef_left, bc_fp_right, bc_ef_right, bc_adj_fp
            )

        print(
            f"Clicked FP: {clicked_x}, WC: {worst_case_effort}d, Adj WC {worst_case_effort_adj}d, BC: {best_case_effort}d, Adj BC: {best_case_effort_adj}d, Likely Effort: {likely_effort}d"
        )

        # Update horizontal lines
        hline_best.set_data(
            [starting_x, clicked_x], [best_case_effort, best_case_effort]
        )
        ...
        hline_best_adj.set_data(
            [starting_x, bc_adj_fp], [best_case_effort_adj, best_case_effort_adj]
        )
        ...
        hline_worst.set_data(
            [starting_x, clicked_x], [worst_case_effort, worst_case_effort]
        )
        ...
        hline_worst_adj.set_data(
            [starting_x, wc_adj_fp], [worst_case_effort_adj, worst_case_effort_adj]
        )
        ...
        def calculate_contingency(
            effort, min_effort, max_effort, max_contingency, k=0.1
        ):
            ...
            contingency = (
                max_contingency * np.exp(-k * (effort - min_effort))
            ) - final_contingency
            return contingency
        ...
        contingency_values = calculate_contingency(
            effort_range,
            best_case_effort_adj,
            worst_case_effort_adj,
            contingency_start,
            k,
        )
        ...
        print(
            f"Minimum Total Effort Value: {min_total_value} at index {min_total_index}"
        )
